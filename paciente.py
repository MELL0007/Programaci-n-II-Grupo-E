...
ventanaPrincipal.configure(bg="#FF9999")
...
def enmascarar_fecha(texto):
    ...
    formato_final="" 
    if len (limpio)>8:
        limpio=limpio[:8] #cuantos campos permite
    if len(limpio)>4:
    ...
    if len(fechaN.get())==10:
        ...
        fecha_nacimiento=datetime.strptime(fechaN.get(),"%d-%m-%Y").date() #formato de muestra
...
def guardar_en_archivo():
    with open ("pacientePeso.txt","w",encoding="utf-8")as archivo: # la "w"es modo de paertura del archivo para guardar(write)
        for paciente in paciente_data:
            archivo.write( #definir como se guarda cada campo
                f"{paciente['Nombre']}|"
...
def cargar_desde_archivo_pacientes():
    try: #accede a datos guardados 
        ...
        open("pacientePeso.txt","w",encoding="utf-8").close()#en caso de error
...
def eliminarPaciente():
    ...
    if seleccionado:
        ...
        if messagebox.askyesno("Eliminar paciente",f"¿Está seguro de eliminar el paciente'{treeview.item(id_item,'values')[0]}'?"):
            ...
            guardar_en_archivo()#guardar cambios en elarchivo 
            cargar_treeview()
...
#PESO (FUNCION AÑADIDA)
pesoPaciente=tk.Label(frame_pacientes,text="Peso")
pesoPaciente.grid(row=8,column=0,sticky="w",padx=5,pady=5)
#Ingresar datos
...
#se actulize al cargar de nuevo
