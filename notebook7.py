...
#eliminar doctor
def eliminarDoctor():
    seleccionadoD=treeview1.selection()
    if seleccionadoD:
        indiceD=int(seleccionadoD[0])
        id_item=seleccionadoD[0]
        
        if messagebox.askyesno("Eliminar doctor",f"¿Está seguro de eliminar el doctor'{treeview1.id_item(id_item,'values')[0]}'?"):
            del doctores_data[indiceD]
            guardar_en_archivoDoc()#guardar cambios en elarchivo 
            cargar_treeview1()
